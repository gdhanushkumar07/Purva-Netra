...
The whole product path works end to end on real data for July 2020: WeatherBench 2 extraction → IMD truth → labels → ensemble-spread baseline → Parquet replay store → API → Forecast Trust Console. It is tested (Python 20/20, Vitest 7/7, Playwright + axe 31/31).
...
| Alignment sanity (§4.4) | Day-1 HRES vs IMD, July 2020: r = __R1__ (Kerala, Vidarbha and Assam plotted in `docs/sanity_day1.png`); r falls to __R10__ by Day 10 |
| Thin slice (July 2020, __NINIT__ inits) | Base rate __BR__ overall, __BRL__ by lead (in-sample thresholds; plumbing only) |
...
6. **Playwright's bundled Chromium** failed to download on this link, so the tests run on the installed Google Chrome (`PW_CHANNEL=chrome`).
